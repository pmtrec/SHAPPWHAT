...
// Importation des icônes utilisées dans l'interface utilisateur
import {
  ChatIcon,
  StatusIcon,
  NewsletterIcon,
...
export async function renderHome() {
  ...
  // Génération de la liste des contacts à afficher
  const contactListElement = renderContactList(contacts);

  // Création de l'élément principal de la page
  ...
  element.innerHTML = `
    <div class="bg-gray-900 h-screen overflow-hidden font-sans">
      <div class="flex h-full">
        ...
        <div id="page" class="flex flex-col w-[400px]">
          <div class="bg-gray-900 p-4 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-8 text-gray-300">
              <!-- Boutons de création et de menu -->
              <button id="NewChatIco" class="cursor-pointer">${NewChatIcon}</button>
              <button id="MenuIco" class="cursor-pointer">${MenuIcon}</button>
            </div>
          </div>
  ...
  // Remplacement du placeholder par la liste des contacts
  ...
  const MenuIco = element.querySelector("#MenuIco");
  if (MenuIco) {
